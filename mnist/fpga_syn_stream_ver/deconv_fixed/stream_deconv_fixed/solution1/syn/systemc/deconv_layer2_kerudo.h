...
#ifndef _deconv_layer2_kerudo_H_
#define _deconv_layer2_kerudo_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "deconv_layer2_kerudo_memcore.h"

SC_MODULE(deconv_layer2_kerudo) {
...
protected:
    ...
    // buffer instances
    deconv_layer2_kerudo_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(deconv_layer2_kerudo) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "deconv_layer2_kerudo_memcore_U_";
            name += szTmp;

            buffer[i] = new deconv_layer2_kerudo_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "deconv_layer2_kerudo.trace");
            // ports
